...
@Component({
  templateUrl: "./html/admin.create.user.data.page.html",
  styles: [`
...
export class AdminCreateUserDataComp implements OnInit {
  ...
  register() {
    if (!this.frmForm.invalid) {
      ...
      this._adminService.registerUser(param).subscribe(res => {
          this.loading = false;
          this.frmForm.reset({
